> 第二階段更新：現行官方名冊、前瞻模擬績效、私人Telegram帳本與精簡通知，詳見 [PHASE2.md](PHASE2.md)。私人帳本需另行部署，尚非盤中交易服務。

# 每日股票觀察清單與台股波段雷達

> 全市場初篩與 Telegram 四類摘要、重大公告去重提醒、週一完整決策報告，見 [MARKET_RADAR.md](MARKET_RADAR.md)。沿用原策略與影子模式，不取代私人帳本或放寬原交易門檻。

> 新版：已加入資料品質、日週K、基本面／籌碼初篩、凍結交易計畫、可靠通知與影子驗證。
> **請先讀 [RADAR.md](RADAR.md)**，其中的排程、測試隔離、資料限制與通知說明優先於下方舊版操作說明。
> 本版預設不推送波段交易建議；催化審核、盤中報價與績效驗證尚未全自動化。
